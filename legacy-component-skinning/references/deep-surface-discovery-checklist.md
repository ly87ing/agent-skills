...
> 默认页只是入口，不是覆盖范围。编辑、详情、step、二级路由、确认弹窗常常才是真正出问题的地方。
...
- 编辑按钮 -> drawer/modal -> 二次确认弹窗
- 列表行“详情/编辑/更多” -> 详情页/二级页 -> 内嵌表单
- step1 -> step2 -> review/confirm
- tab A 正常 -> tab B/C 旧皮肤或布局错乱
- 高级区展开 -> 深层表单 / 帮助文案 / warning banner
...
- 只点显眼按钮，没覆盖 row action、tab、route jump
