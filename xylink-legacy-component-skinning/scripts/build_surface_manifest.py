...
def parse_structured(value: str, expected_parts: int, flag: str) -> list[str]:
    ...
    if len(parts) != expected_parts or any(not part for part in parts):
        raise argparse.ArgumentTypeError(
            f"{flag} requires {expected_parts} non-empty fields separated by |, received: {value!r}"
        )
    return parts
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Interactive surface definition. Example: click|Change password|modal|Account settings header action",
    )
    parser.add_argument(
        "--exception",
        action="append",
        ...
        help="Exception surface definition. Example: backend-unavailable|page-banner|browser request blocking|Login page top alert",
    )
    parser.add_argument(
        "--layout-case",
        action="append",
        ...
        help="Content/layout case definition. Example: toolbar|select trigger|long selected value|primary copy stays readable|only 3 chars visible|before=`TODO` / after=`TODO`|panel width must increase",
    )
    parser.add_argument(
        "--breakpoint",
        action="append",
        ...
        help="Breakpoint coverage. Example: desktop|before=`TODO` / after=`TODO`|1440px review",
    )
    parser.add_argument(
        "--unverified",
        action="append",
        ...
        help="Unverified item. Example: second-step editor|local env not booted|revisit after env is available",
    )
