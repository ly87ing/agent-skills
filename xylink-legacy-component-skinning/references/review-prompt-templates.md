...
> 这些模板用于人工发起 XYLINK 口径的 review / 验收 / 补漏回看。按场景复制，替换方括号内容即可。

## 实施补漏 review prompt

适用场景：

- 默认页已经较像 XYLINK，但深层链路还没收敛
...
```text
继续按 xylink-legacy-component-skinning 的实施路径处理这个页面，不要先做 review。

范围：
- 页面/路由：[页面或路由]
- XYLINK 参考源：[demo URL / 截图包]
- 已看过的范围：[默认页、首层弹窗等]
...
- 把异常态一起按 XYLINK 口径回看：[权限不足 / 后端不可用 / 校验失败 / 空数据异常等]
...
- 不要只说“默认页已经比较像 XYLINK”，要明确哪些深层页面和状态已看，哪些还没看
```
...
- 已有一轮实现，需要判断能不能对外说“已向 XYLINK 收敛”
- 需要复核共享层收敛、深层页面覆盖和真实观察证据

模板：

```text
按 xylink-legacy-component-skinning 的验收路径 review 这次页面调整，判断现在能不能说“已经向 XYLINK 收敛”。
...
- 内容展示和页面布局是否真的向 XYLINK 收敛，而不是只看颜色、边框、圆角
- 是否存在应该写进 manifest 但没有写的范围
- 是否有无法真实查看却仍被说成“已通过/已收敛”的状态
...
```text
我现在只能提供当前 diff 和少量截图，没有完整浏览器回看证据。请按 xylink-legacy-component-skinning 先做方向性判断，但不要直接说“已向 XYLINK 收敛/已通过”。
